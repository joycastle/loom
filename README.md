...
**① Claude Code 插件(最省事)** — 装完 skill 自动可用,首次使用会引导装底层 CLI:
...
**② 任意终端一行**(codex / cursor 或普通终端,零 pip、零打包):
...
```bash
git clone https://github.com/joycastle/loom.git ~/Documents/loom
cd ~/Documents/loom && ./install.sh      # 装 CLI 到 PATH + 引导配置 + 装 skill
```

三条都会把 `loom` 软链到 PATH 并把 loom 技能装进在场的 AI 助手;之后日常只需一条 `loom sync`(上云加 `--push`)。
