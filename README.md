...
装 loom = 两样东西落到你机器:**`loom` 命令**(纯标准库 Python CLI,真正干活的)+ **loom 技能**(装进你的 AI 助手,让它会用 `loom`)。按你手头的工具挑一种:

**① 用 Claude Code —— 装成原生插件(最省事)**

在 **Claude Code 会话里**输入(是斜杠命令,不是终端命令):
...
技能立即可用;**首次**让它干活时,它会自动帮你把底层 `loom` 命令装到 PATH——你什么都不用先装。

**② 用 Codex / Cursor / 任意终端 —— 一行装**

在终端(含 Codex、Cursor 内置的终端)跑这一行,自动装好 `loom` 命令 + 把技能装进在场的所有 AI 助手(Claude / Codex / Cursor / CodeBuddy),零 pip、零打包:
...
```bash
git clone https://github.com/joycastle/loom.git ~/Documents/loom
cd ~/Documents/loom && ./install.sh      # 装 loom 命令到 PATH + 引导配置 + 装技能进 AI 助手
```

装完:日常只需一条 `loom sync`(上云加 `--push`);想看界面就 `loom serve` 浏览器打开管理页。
...
欢迎 issue / PR:新采集器(实现 `collect(cfg, since)` 即插即用)、新语言的文档、坑清单补充。
改代码后请跑 `python3 -m pytest tests/test_loom.py`(零依赖,应全绿)。


---
**给拿到本项目的 AI 助手**:先读 [`AGENTS.md`](./AGENTS.md)(或 `CLAUDE.md`),首次上手照 [`ONBOARDING.md`](./ONBOARDING.md) 执行。
**让任意项目里的 AI 都能查你的台账**(全局规则一段话接入):见 [`docs/ai-global-access.md`](./docs/ai-global-access.md)。
