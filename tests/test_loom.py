...
class CodexFeishuBridgeCollectorTest(unittest.TestCase):
    ...
    def test_collects_every_topic_where_self_actually_spoke(self):
        ...
        self.assertEqual(first["detail"]["participants"], ["小李", "老王"])
        self.assertIn("老王: 请分析原因", first["detail"]["body"])
        ...
        bot_only = self._message("om_self", self.me, "老王", "只问机器人",
                                 "2026-07-22 16:00")
        ...
        mentioned_only = self._message("om_mention", "ou_other", "小李", "@老王 看一下",
                                       "2026-07-22 17:00")
        ...
        self.assertEqual(own_entries[0]["detail"]["participants"], ["老王"])
    ...
    def test_complete_body_is_not_silently_truncated(self):
        ...
        root = self._message("om_long", self.me, "老王", content,
                             "2026-07-22 18:00")
    ...
    def test_since_filters_topic_days(self):
        root = self._message("om_root", "ou_other", "小李", "问题", "2026-07-01 09:00")
        root.update({"thread_id": "omt_topic", "thread_replies": [
            self._message("om_me", self.me, "老王", "处理", "2026-07-02 10:00")
        ]})
        entries = feishu_bridge_col.topic_entries(
